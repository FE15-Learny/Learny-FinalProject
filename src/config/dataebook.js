export const ebooklist = [
  {
    id: 1,
    title: "Beginning HTML, XHTML, CSS, and JavaScript",
    category: "website",
    subCategory: ["code", "programming", "development"],
    description:
      "This beginner guide shows you how to use XHTML, CSS, and JavaScript to create compelling Web sites. While learning these technologies, you will discover coding practices such as writing code that works on multiple browsers including mobile devices, how to use AJAX frameworks to add interactivity to your pages, and how to ensure your pages meet accessible requirements.",
    authorName: "Jon Duckett",
    authorAvatar:
      'https://i.ibb.co/SmPv880/jon-ducket.jpg alt="jon-ducket" border="0"',
    createdAt: "Dec 30, 2009",
    cover: 'https://i.ibb.co/8YDB4sj/cover1.png alt="cover1" border="0"',
  },
  {
    id: 2,
    title: "UX AND UI STRATEGY: A STEP BY STEP GUIDE ON UX AND UI DESIGN",
    category: "design",
    subCategory: ["ui/ux", "design", "guide"],
    description:
      "A step by step Guide on UX and UI design This book analyzes how Don Norman originated the word “User Experience Design” in the 1990s and it means a person’s perception or feeling towards using a product, service, website or software.",
    authorName: "Pamala B. Deacon",
    authorAvatar: 'https://i.ibb.co/0s6LsLY/pamala.jpg alt="pamala" border="0"',
    createdAt: "Dec 14, 2020",
    cover: 'https://i.ibb.co/jTWMtm0/cover2.jpg alt="cover2" border="0"',
  },
  {
    id: 3,
    title: "Modern Full-Stack Development",
    category: "development",
    subCategory: ["full stack", "website", "development"],
    description:
      "JavaScript on both sides of the fence to make our coding lives better. We’re going to touch on several other tools that relate to all of this including Babel and Webpack.",
    authorName: "Frank Zammeti",
    authorAvatar: 'https://i.ibb.co/CHtR876/frank.jpg alt="frank" border="0"',
    createdAt: "Jan 01, 2020",
    cover: 'https://i.ibb.co/nwV19tz/cover3.png alt="cover3" border="0"',
  },
  {
    id: 4,
    title: "Designing and Prototyping Interfaces with Figma.",
    category: "design",
    subCategory: ["design", "ui/ux", "figma"],
    description:
      "Being a driving force in the design tools market, Figma makes everything easier by bringing unique innovations and opening up real-time collaboration possibilities, so it comes as no surprise that so many designers decide to switch from other tools to Figma.",
    authorName: "Fabio Staiano",
    authorAvatar: 'https://i.ibb.co/wd3zS68/fabio.jpg alt="fabio" border="0"',
    createdAt: "March 16, 2022",
    cover: 'https://i.ibb.co/BsZj70R/cover4.png alt="cover4" border="0"',
  },
  {
    id: 5,
    title: "The GitHub Book",
    category: "development",
    subCategory: ["tools", "github", "tutorial"],
    description:
      "Learn how to use GitHub as a power user. Learn the ins and outs of the GitHub API using many of the most popular programming languages. Supercharge your development with command line shortcuts. ",
    authorName: "Chris Dawson & Ben Straub",
    authorAvatar: 'https://i.ibb.co/g9s7Z5z/ben.jpg alt="ben" border="0"',
    createdAt: "July 28, 2015",
    cover: 'https://i.ibb.co/D9dJjJv/cover5.png alt="cover5" border="0"',
  },
  {
    id: 6,
    title: "React.js Essentials",
    category: "development",
    subCategory: ["react.js", "website", "development"],
    description:
      "If you're a front-end developer with knowledge of jQuery and its libraries, along with frameworks, such as Angular.JS and Backbone.JS, or native JavaScript development, and you wish to use the fastest web user interface library there is, then this book is ideal for you.",
    authorName: "Artemij Fedosejev",
    authorAvatar:
      'https://i.ibb.co/28KTBYd/artemij.jpg alt="artemij" border="0"',
    createdAt: "August 27, 2015",
    cover: 'https://i.ibb.co/tP24FYj/cover6.png alt="cover6" border="0"',
  },
  {
    id: 7,
    title: "The Graphic Design Book: A Comprehensive Guide for Beginners",
    category: "design",
    subCategory: ["branding", "design", "graphic design"],
    description:
      "This book is a guide which provides information for individuals who are seeking to become expert graphic designers.",
    authorName: "Festus Atiba",
    authorAvatar: 'https://i.ibb.co/r57c4d7/festus.jpg alt="festu" border="0"',
    createdAt: "Feb 15, 2021",
    cover: 'https://i.ibb.co/BBL3pgB/cover7.png alt="cover7" border="0"',
  },
];
